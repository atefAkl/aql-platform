...
export interface BadgeProps {
    children: ReactNode;
    variant?: 'pending' | 'approved' | 'suspended' | 'active' | 'provisioned' | 'default' | 'info' | string;
    className?: string;
}
...
export const Badge: React.FC<BadgeProps> = ({
    ...
    const variantStyles: Record<string, string> = {
        ...
        provisioned: 'bg-blue-500/10 text-blue-500 border-blue-500/20 dark:bg-blue-500/10 dark:text-blue-400 dark:border-blue-500/30',
    ...
    return (
        <span className={`${baseStyles} ${variantStyles[variant] || variantStyles.default} ${className}`}>
            <span className={`w-1.5 h-1.5 rounded-full ${
                ...
                (variant === 'active' || variant === 'provisioned') ? 'bg-blue-500' :
                variant === 'suspended' ? 'bg-rose-500' : 'bg-slate-400'
            }`} />
            {children}
        </span>
    );
